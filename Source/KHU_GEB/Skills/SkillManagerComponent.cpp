...
bool USkillManagerComponent::IsFormChangeLocked() const
{
	for (auto& It : Equipped)
	{
		if (It.Value && It.Value->IsBlockingFormChange())
		{
			return true;
		}
	}
...
bool USkillManagerComponent::IsAiming() const
{
	for (auto& It : Equipped)
	{
		if (It.Value && It.Value->IsAiming())
		{
			return true;
		}
	}
...
void USkillManagerComponent::HandleAimMoveInput(const FVector2D& Input)
{
	for (auto& It : Equipped)
	{
		if (It.Value && It.Value->IsAiming())
		{
			It.Value->HandleAimMoveInput(Input);
		}
	}
}
